...
extern void switch_to_kernelland();
...
trap_dispatch(struct trapframe *tf) {
    ...
    case T_SWITCH_TOU:
        if (tf->tf_cs != USER_CS) {
            ...
            user_tf.tf_esp = (uint32_t)tf + sizeof(struct trapframe) - 8;
    ...
    case T_SWITCH_TOK:
        if (tf->tf_cs != KERNEL_CS) {
            ...
            kern_tf = (struct trapframe *)(tf->tf_esp - (sizeof(struct trapframe) - 8));
